...
| CON-005 | 자동 발견 범위는 IPv4 동일 LAN으로 제한하여야 한다.(삭제 고민) |
...
| `rtsp_port` | integer | `1..65535`, `bool` 제외 (확인 필요) |
...
``` json
{
  ...
  "rtsp_port": 8554,
  "rtsp_path": "/stream"
} (device_id는 mac으로, mesage_id는 삭제 고려)
```
...
| ID | 요구사항 |
| --- | --- |
| FR-MSG-001 | 시스템은 유효한 메시지를 compact(확인 필요) UTF-8 JSON bytes로 직렬화하여야 한다. |
| FR-MSG-002 | 시스템은 UDP payload를 엄격한 UTF-8과 JSON 객체로 역직렬화(확인필요)하여야 한다. |
...
| FR-MSG-005 | 잘못된 네트워크 입력은 listener 밖으로 예외를 전파하지 않아야 한다. (확인필요)|
| FR-MSG-006 | 메시지 ID를 지정하지 않으면 고유 ID를 생성하여야 한다. (삭제 고려)|
...
| FR-SND-006 | DETAIL ACK는 실제 pending DETAIL ID와 전송 peer(표현 적절성 검토)가 모두 일치할 때만 수락하여야 한다. |
...
| FR-SND-011 | 잘못된 endpoint나 비-JSON details는 network thread 시작 전에 거부하여야 한다. (확인 필요)|
...
| FR-RCV-004 | 수신기는 새롭고 재검증이 필요한 ADVERTISE에 대해 RTSP probe(확인 필요) 를 예약하여야 한다. |
...
| FR-RCV-011 | probe 실패 장비도 전체 상태에는 \`rtsp\_connected=false\`로 보존하여야 한다(확인 필요). |
| FR-RCV-012 | probe 성공 장비만 발견 성공 결과와 최초 callback으로 전달하여야 한다. (확인 필요)|
| FR-RCV-013 | 연결된 장비의 details가 실제로 바뀌면 최신 snapshot으로 callback을 다시 호출하여야 한다. (확인 필요) |
...
| FR-RCV-017 | \`get\_devices()\`는 \`device\_id\`를 키로 한 전체 최신 상태의 깊은 복사본을 반환하여야 한다. (확인 필요)|
...
| FR-RTSP-005 | 완전한 \`CRLF CRLF\` 응답 header를 받아야 성공 판정을 수행하여야 한다. (확인 필요)|
| FR-RTSP-006 | 정확한 \`RTSP/2.0\` status line, \`2xx\`, \`CSeq: 1\`을 모두 만족할 때만 성공하여야 한다. (확인 필요) |
