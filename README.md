...
- 메시지 재전송, 중복 제거, replay 방지, 패킷 재정렬
...
| `timeout` | `2.0` | 두 ACK 교환 전체의 최대 시간 |
...
| 인자 | 기본값 | 의미 |
| --- | --- | --- |
| `timeout` | 필수 | UDP 교환과 RTSP probe 전체의 최대 시간 |
...
| `tests/test_sender.py` | 6 | 강제 broadcast, ACK 상관관계, peer, 사전 검증 |
| `tests/test_receiver.py` | 3 | malformed 입력, peer/device 고정, timeout |
| `tests/test_connecter.py` | 7 | URI, OPTIONS/CSeq, 2xx 경계, 오류와 timeout |
| `tests/test_e2e.py` | 3 | 전체 성공, RTSP 실패, probe 예외 격리 |
